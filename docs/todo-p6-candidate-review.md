...
- 当前仍保持 10 个实施主题，但结构调整为 8 个近期主题和 2 个条件主题；`P6-101`、`P6-104`、`P6-129` 不再作为独立主题。
- 近期队列约 18-30 个工作日；条件主题只在真实标准包升级或消费仓库出现后增加约 5-8 个工作日。

## 近期值得做

| 顺序 | 主题 | 承接编号 | 价值判断 | 估算 |
| --- | --- | --- | --- | --- |
| 1 | 稳定引用、历史别名与 Evidence claim 真实性修复 | P6-190 | 修复已声明契约与实际行为偏差，是后续 AI 评审可信度前置条件 | 3.5-6 天 |
| 2 | 确定性数标命名解析与缩写治理 | P6-189 | 一次增强推荐、检索、标准问答、候选和 AI Context，且可复用现有 glossary | 3-5 天 |
| 3 | 推荐质量与 AI 场景回归 | P6-120、P6-126、P6-133、P6-162 | 防止命名评分和关键 AI 链路静默退化，不新增回放框架 | 1-2 天 |
| 4 | 字段库性能与密集操作 | P6-86、P6-122、P6-145、P6-151 | 已有全量加载和固定结果上限的代码证据，直接影响性能与结果可达性 | 2-3 天 |
| 5 | Finding/Evidence 与 AI/PR 评审闭环 | P6-191 | 统一现有 lint、post-check、PR 评论和证据包，不重造 Reviewer | 3-5 天 |
| 6 | `.dataspec/config.json` Schema 与运行摘要 | P6-101、P6-103、P6-118、P6-119、P6-137、P6-173 | 低成本补稳定配置入口，并吸收重复的独立运行指纹主题 | 0.5-1 天 |
| 7 | 标准候选来源管道 | P6-110、P6-111、P6-112、P6-113、P6-124、P6-136、P6-142 | 第一版只接未知词/歧义缩写等一个证据充分来源 | 2-3 天 |
| 8 | 演示项目 dry-run 清理与重建 | P6-85 | 补本地维护闭环，但必须保留事务、预览和安全配置边界 | 3-5 天 |
...
1. 不再按 P6 编号顺序自动开发，先修契约真实性，再建设命名解析和评审闭环。

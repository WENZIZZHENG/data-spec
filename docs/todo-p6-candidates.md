...
- `P6-189`、`P6-120`、`P6-86`、`P6-191` 和 `P6-137` 已完成并移入 [P5/P6 完成归档](archive/todo-completed-p5-p6.md)；确定性 token evidence 和推荐回归 fixture 继续作为 `P6-111` 的输入。
- 当前共 4 个实施主题，承接 22 个原候选编号；其中 2 个进入近期队列，2 个等待业务触发。
...
### 4. P6-123：标准消费 Schema、Contract-as-Code 与轻量锁定
...
1. `P6-111`：复用命名解析结果接入第一个候选来源。
2. `P6-85`：最后收口演示项目重建和本地维护边界。

近期 2 个主题串行约 5-8 个工作日；两个暂缓主题仅在触发条件成立后增加约 5-8 个工作日。

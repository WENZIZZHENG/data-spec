...
- `P6-189` 已完成并移入 [P5/P6 完成归档](archive/todo-completed-p5-p6.md)，其确定性 token evidence 作为 `P6-120` 和 `P6-111` 的输入。
- 当前共 8 个实施主题，承接 36 个原候选编号和 1 个第三轮新增编号；其中 6 个进入近期队列，2 个等待业务触发。

## 近期队列

### 1. P6-120：推荐质量与 AI 场景回归基线
...
### 3. P6-191：统一 Finding/Evidence 与 AI/PR 评审闭环
...
### 4. P6-137：`.dataspec/config.json` Schema 与编辑器提示
...
### 8. P6-123：标准消费 Schema、Contract-as-Code 与轻量锁定
...
1. `P6-120`：立即固化命名推荐与 AI 链路回归基线。
2. `P6-86`：解决已有数据证明的字段库性能和结果截断。
3. `P6-191`：复用已完成的 evidence resolver，统一 Finding/Evidence 和 PR 评审。
4. `P6-137`：以低成本补稳定配置入口并吸收运行摘要。
5. `P6-111`：复用命名解析结果接入第一个候选来源。
6. `P6-85`：最后收口演示项目重建和本地维护边界。

近期 6 个主题串行约 11.5-19 个工作日；两个暂缓主题仅在触发条件成立后增加约 5-8 个工作日。

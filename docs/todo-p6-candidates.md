...
- 当前共 10 个实施主题，承接 36 个原候选编号和 3 个本轮新增编号；其中 8 个进入近期队列，2 个等待业务触发。

## 近期队列

### 1. P6-190：稳定引用、历史别名与 Evidence claim 真实性修复

- 类型：契约修复、数据一致性、AI 输出可信度；进入 OpenSpec / SDD full。
- 为什么值得做：主规格已承诺历史别名召回和 evidence claim 验证，但当前实现只读取当前别名，任意 evidence URI 也不会查询真实证据；继续扩展 AI 评审会放大不可信结果。
- 已有基础：字段变更日志、标准快照、stableRef/canonicalRef、AI output post-check、Evidence Package 和 GitHub inline review 已存在。
- 近期范围：从变更日志或快照派生字段历史名并接入 ref resolve、推荐和检索；增加 project-scoped evidence resolver，区分存在、缺失、跨项目和不可验证；同步冲突的 inline/summary review 主规格。
- 验收：旧字段名能解析到当前 canonicalRef 并返回来源证据；真实 evidence claim 可通过，伪造、跨项目或不可验证 claim 得到稳定诊断；相关主规格不再互相冲突。
- OpenSpec 交接：建议 change-id 为 `fix-stable-reference-evidence-truthfulness`；依赖现有变更日志、标准快照和项目级引用解析，不依赖其他待办；重点更新 `stable-standard-references`、`field-standard-search`、`ai-output-postcheck`、`ai-evidence-package`、`github-inline-review`，保持既有响应字段兼容、证据解析严格限定项目范围且不迁移或回填不可推导历史数据；验证至少覆盖后端、前端、CLI/MCP/tools、相关契约 fixture、`openspec validate <change-id> --strict`、`openspec validate --all` 和 `git diff --check`。
- 边界：不建设企业审计平台，不新增第二种 evidence 格式，不回填无法可靠推导的历史记录。
- 估算：3.5-6 个工作日。

### 2. P6-189：确定性数标命名解析与缩写治理
...
### 5. P6-191：统一 Finding/Evidence 与 AI/PR 评审闭环
...
### 6. P6-137：`.dataspec/config.json` Schema 与编辑器提示
...
### 10. P6-123：标准消费 Schema、Contract-as-Code 与轻量锁定
...
1. `P6-190`：先修复已声明契约的真实性，避免新评审能力建立在假 evidence 上。
2. `P6-189`：统一命名解析、缩写治理和 token evidence。
3. `P6-120`：立即固化命名推荐与 AI 链路回归基线。
4. `P6-86`：解决已有数据证明的字段库性能和结果截断。
5. `P6-191`：在 evidence resolver 稳定后统一 Finding/Evidence 和 PR 评审。
6. `P6-137`：以低成本补稳定配置入口并吸收运行摘要。
7. `P6-111`：复用命名解析结果接入第一个候选来源。
8. `P6-85`：最后收口演示项目重建和本地维护边界。

近期 8 个主题串行约 18-30 个工作日；两个暂缓主题仅在触发条件成立后增加约 5-8 个工作日。

...
本文件保存 TODO.md 中 P0-P4 已完成第一版条目的详细背景、方案、验收和参考资料。主 TODO.md 仅保留当前仍需行动的摘要，避免 AI 或人工排期把历史缺口误判为未实现。
...
- 已完成能力：已补齐 zip 打包、`.dataspec/` 目录约定、`AGENTS.md` 片段、JSON Schema、few-shot 示例和 AI prompt 模板。
...
- 已完成能力：已补齐批量文件扫描、stdin 输入、JSON 输出、退出码约定和本地命令入口；安装体验后续由 P4-3/P5-2 继续增强。
...
- 已完成能力：已补齐 MCP resources/prompts/tools 映射、项目选择和错误输出协议；`explain_lint_issue` 与更细鉴权作为后续增强。
...
- 已完成能力：已补齐 `aliases`、`sensitive`、`status`、`exampleValue`、`codeSetId` 等字段元数据，并同步到 AI 字段目录。
...
- 已完成能力：已补齐统一 `rules.yaml` 命名规则导出、后缀/前缀类型规则和结构化规则配置页面；拼音/无意义词检测留 P5-6。
...
- 已完成能力：已补齐建表 Prompt 模板、项目标准字段填充和 SQL 检查后的修正 Prompt 生成能力。
...
- 已完成能力：前端已调用真实 `/api/lint`，并按后端契约展示统计、问题列表、修正 SQL 和检查记录。
...
- 已完成能力：已通过 OpenAPI 生成 `schema.ts` 统一类型来源，并提供 `pnpm check:api` 防漂移入口。
...
- 已完成能力：已补齐 `COMMENT ON TABLE/COLUMN` 解析、表名 snake_case 规则和基础定位；更完整 source map 留 P5-5。
...
- 已完成能力：项目列表、当前项目选择、字段库 CRUD 和规则配置 CRUD 已接后端。
...
- 已完成能力：已补齐按业务描述查找标准字段、推荐字段名、别名匹配、置信度和命中原因输出；质量增强留 P5-6。
...
- 已完成能力：`suggestion`、`replacement`、`before`、`after`、`confidence` 等结构化修复字段已接入核心 lint 输出。
...
- 已完成能力：已补齐面向 API/CLI/MCP 的 DDL 生成接口和 lint 自检闭环。
...
- 已完成能力：已补齐 SQL 检查记录、`fixedSql`、结构化修复建议和前端历史记录/详情查看。
...
- 已完成能力：创建项目时已支持内置 standards 导入、去重和错误反馈，且可选择空白项目。
...
- 已完成能力：已补齐 DDL 生成服务、前端预览、复制和 `.sql` 下载能力。
...
- 已完成能力：已补齐业务项目 `.dataspec/` 目录约定、manifest、README、示例、prompt 和 agent 指令片段。
...
- 已完成能力：Markdown/HTML 数据字典、概览统计、字段关系和 Mermaid ERD 已补齐第一版。
...
- 已完成能力：已补齐 `.xlsx` 模板、字段/代码集批量导入导出、导入预览、错误报告和确认导入。
...
- 已完成能力：已补齐标准变更日志表、查询 API 和 before/after JSON 自动记录。
...
- 已完成能力：已补齐检查记录、字段命中率计算、趋势统计和轻量工作台首页。
...
- 已完成能力：已补齐 SQL DDL 反向导入预览，并在 P4-11/P4-12 补齐数据库直连流程；二次差异模型留 P5-3。
...
- 已完成能力：已补齐 MySQL 类型、长度、COMMENT、AUTO_INCREMENT、tinyint、datetime、反引号等兼容解析与测试。
...
- 已完成能力：已补齐 CLI 批量扫描、退出码约定和 GitHub Actions 示例。
...
- 已完成能力：已补齐 PR 评论式 Markdown 输出和重复评论更新策略；文件级 inline 定位留 P5-5。
...
- 后续增强：当前已提供启发式 `line/column/sourceStart/sourceEnd`；统一 source map 和 GitHub inline comment 基础留 P5-5。
...
- 已完成能力：已补齐 unified diff 输出和前端差异视图；side-by-side 编辑体验留 P5-8。
...
- 已完成能力：已补齐项目级默认配置和更短的默认项目使用方式；npm bin/doctor 体验留 P5-2。
...
- 已完成能力：已补齐结构化 UI 表单、参数摘要、JSON 预览和兜底编辑。
...
- 已完成能力：已补齐本地 OpenAPI 契约防漂移脚本 `pnpm check:api`。
...
- 状态：已完成第一版，导入预览已返回行级 dry-run 明细、字段级 before/after diff、重复别名和未知引用冲突原因；前端导入页已展示动作、状态、原因和可展开字段差异。
- 为什么做：当前 Excel 能预览和确认导入，但批量维护标准时还需要更细的冲突解释，降低误改风险。
- 已有基础：`.xlsx` 模板、导出、预览、新增/更新/冲突统计和确认导入已完成。
- 已完成能力：已补齐字段级 diff、重复别名检测、冲突原因和导入 dry-run 明细；枚举覆盖策略可按实际使用继续细化。
...
- 状态：已完成第一版，后端已新增 HTML 数据字典和 Mermaid ERD/关系图预览/下载接口，前端生成器页已支持 HTML iframe 预览、ERD 文本预览和两类文件下载。
- 为什么做：Markdown 已适合 Git 管理，但团队分享和浏览关系时 HTML/ERD 更直观。
- 已有基础：Markdown 数据字典已包含字段、数据域、枚举、模板和元数据。
- 已完成能力：已补齐可浏览 HTML、字段/代码集/模板关系图和下载入口。
...
- 后续增强：SQL Server 等其他方言尚未按真实样例扩展；当前先保持 PostgreSQL 与常见 MySQL 兼容。
...
### P4-9：个人/小团队安全基线
- 状态：已完成第一版，已新增可配置 API Token 安全模式、token hash 存储、项目级授权检查、前端 token 登录/退出、CLI/MCP token 透传和变更日志操作者记录。
- 为什么做：项目从个人自用走向小团队时，需要最小身份、项目边界和 API token，避免标准数据被误改。
- 已有基础：项目空间、变更日志和后端统一 API 已存在。
- 后续增强：轻量 token 管理页面留 P5-9；角色矩阵、审批/发布流程和复杂 RBAC 仍作为明确不做边界。
- 落地产物：最小用户身份、项目级访问边界、CLI/MCP/API token、变更日志记录操作人。
- 验收标准：不同 token 只能访问授权项目；字段/规则变更日志能看到操作者。
- 边界：不做审批流、发布流、复杂 RBAC 或企业治理驾驶舱。

### P4-10：演示项目与首次使用引导
- 状态：已完成第一版，已新增一键创建/复用 `DataSpec 演示项目`，自动补种内置字段、核心规则、订单表模板和示例 SQL；前端工作台/项目列表已提供演示项目入口，并串联 DDL 生成、SQL 校验、AI Context 导出和反向导入路径。
...
- 后续增强：第一版已补齐 demo project、一键示例数据和推荐路径；后续如需要，可再增加更细的引导完成状态。
...
- 已完成能力：已补齐数据库连接配置、连接测试、JDBC metadata 读取、schema/table 选择、直连导入预览和确认写入字段库流程。
...
- 已完成能力：已补齐步骤条、连接状态反馈、表搜索/全选/清空、候选字段分组勾选、确认导入弹窗和导入后跳转字段库入口。

...
public class AiCapabilityCatalogServiceImpl implements AiCapabilityCatalogService {
    ...
    public static final String CATALOG_VERSION = "2026.06.28";
    ...
    private static Map<String, AiCapabilityEntry> builtIns() {
        ...
        add(map, cap(
                ...
                "从现有 PostgreSQL/MySQL metadata 预览字段候选、差异，并确认导入标准候选。",
                true, "WRITES_DATASPEC_STANDARD",
                list("projectId", "databaseType", "host", "databaseName", "username"), list("schemaName", "tableNames"),
                list("reverse-import-preview", "reverse-import-compare-result"),
                list("POST /api/reverse-import/database/preview", "POST /api/reverse-import/database/compare", "POST /api/reverse-import/database/import"),
                list("dataspec workflow show reverse-import-standards --format json"),
                list(), list(),
                list("/reverse-import"),
                list(), list("reverse-import-standards"),
                list("reverse-import"),
                examples("Workflow", "dataspec workflow show reverse-import-standards --format json", null),
                list("使用只读数据库账号", "不要保存密码到仓库", "确认导入前查看候选和来源"),
                list("先 preview/compare，再把候选纳入标准候选 Inbox 或确认导入。"),
                "README.md#sql-规范闭环"
        ));
        add(map, cap(

...
DataSpec 用于统一数据库字段命名、数据类型、注释、枚举、表模板、业务对象和建表规范。当前已形成个人/小团队可用的字段标准工作台，并提供任务式入口、全局命令面板、最近操作续跑、统一前端数据状态、项目活动时间线、标准健康趋势、标准质量门禁、领域 Starter Kit、业务术语表、业务对象与表结构标准、标准使用示例与反例库、合成标准样例生成、标准驱动测试数据包、消费端兼容验收套件、多源契约候选预览、自然语言需求草案、Explain Trace、SQL 校验、DDL 生成、标准候选采纳、数据字典、Excel 导入导出、项目备份恢复、标准复用包、AI Context、AI 会话启动包、AI 任务卡、AI 能力清单、AI 任务模式、AI 回放与反馈、AI 批量任务交付包、AI 任务失败恢复、AI 执行证据包、AI 交接证据看板、API Token 安全基线与管理页、单机轻量幂等写保护、CLI、MCP 和 GitHub PR Review 等能力。
...
- 基于表模板和业务对象表结构标准生成 PostgreSQL DDL preview，返回主键、唯一键、索引、外键、跳过原因、策略说明和 lint 自检摘要；preview 只生成 SQL 文本和证据，不连接业务库、不执行迁移。
- Markdown/HTML 数据字典，包含概览统计、字段与数据域关系、枚举、模板、个人版字段元数据和 Mermaid 关系图。
- 个人工作台，展示任务入口、最近任务、字段数、代码集数、规则数、检查数、字段命中率、最近检查、问题趋势和项目活动时间线。
- 全局命令面板，支持顶部入口或 `Ctrl/⌘+K` 搜索页面、项目动作和最近 SQL 检查/反向导入/AI 作业续跑命令；本地最近命令只保存安全 route/query，不保存 SQL 原文、token、payload 或数据库连接信息。
...
- AI Context zip 导出，包含 `.dataspec/` 目录、字段目录 JSON Schema、项目级业务术语表、表结构标准、字段语义规则、字段知识卡、指标口径、规则、项目规则例外、标准使用示例/反例、prompt、workflow recipes、示例 SQL 和 `AGENTS.md.fragment`；支持按字段、数据域、标签、表、业务对象、表模板、状态和关键词导出按需包，也支持按历史标准快照导出可复现上下文，并可写入业务仓库 `.dataspec/context/` 离线缓存。字段目录会导出生命周期状态、可选替代字段/说明、命名翻译偏好和枚举生命周期摘要；`.dataspec/table-standards.json` 会导出业务对象、模板结构标准、关系 edge、主键/唯一键/索引/外键和策略摘要；`.dataspec/field-knowledge-cards.json`、`.dataspec/field-semantics.json` 和 `.dataspec/metrics.json` 只作为 metadata guidance，不执行真实计算或 SQL；manifest 会携带最近标准包应用摘要、漂移计数和新增语义 artifact 的 count/truncation。
...
- MCP 会话状态记忆，提供只读 `session-state` resource 和 `get_session_state` tool，聚合本地 `.dataspec/config.json`、上下文缓存元数据、当前 profile、最近 task run 入口、安全默认值和 nextActions；不写会话文件、不读取业务数据行、不把状态当权限依据。
...
- 标准质量门禁评估复用字段质量、最近标准健康覆盖率快照和可选 SQL lint 摘要，给 AI/CI 返回稳定 JSON；第一版只显式检查，不做审批流，也不默认阻断本地保存。
...
- 字段推荐、字段标准检索和字段语义知识 API/CLI/MCP；启用的业务术语表、命名翻译、禁用翻译、字段语义规则、枚举 lifecycle 和指标口径会参与“会员手机号”“订单费用”等自然语言 query 的确定性匹配或风险提示；推荐和检索结果包含轻量 evidence/semanticSummary 数组，便于 AI 读取来源、置信度、字段边界和文档引用。
- DDL 生成 API/CLI/MCP，保留 `structureSummary` 结构标准摘要，供 AI 判断哪些约束已应用、哪些提示被跳过、哪些策略需要人工确认。
...
- 标准驱动测试数据包 API/CLI/MCP：`POST /api/test-data/package/generate`、`test-data generate` 和 `generate_test_data_package` 可按字段标准、枚举、格式约束、敏感标记和轻量对象提示生成 deterministic valid/invalid/boundary cases、JSON mock、CSV 行和 SQL seed 草稿；输出声明只读、不写项目、不读取真实业务数据行、不调用外部 LLM。
- 消费端兼容验收套件：`consumer-compat check`、MCP `consumer-compatibility-suite` resource 和 `check_consumer_compatibility` tool 会本地校验 Schema Registry、AI Context、CLI JSON、MCP descriptor/resource/tool、CLI/MCP fixture 和测试数据包 golden payload，输出 adapter results、breaking rules、diagnostics 和 nextActions。
...
- CLI 支持业务仓库初始化 `init`、AI 会话启动包 `bootstrap`、AI 任务卡 `task-card`、环境自检 `doctor`、版本兼容检查 `compat check`、消费端兼容检查 `consumer-compat check`、capability catalog、workflow recipes、AI task run 查询、质量门禁检查、单文件 lint、批量 `lint-files`、变更感知 `changed/lint-changed`、本地 SQL 检查 hook 安装 `install-hook`、业务代码字段引用索引 `index-refs`、字段变更业务代码 Patch Plan `code-patch plan`、AI 批量交付包文件输出、PR inline/汇总评论式 `review-pr`、AI Context 导出、AI Context 预算计划、历史快照 Context 导出、字段推荐、字段标准检索、DDL 生成、合成标准样例生成、标准驱动测试数据包、契约候选导入预览和数据库 schema plan 只读预览。
- MCP Server 暴露 DataSpec resources、resource templates、version compatibility、session bootstrap、session state、consumer compatibility suite、agent guidance pack、task card tools、capability catalog、workflow recipes、AI task runs、prompts、核心 tools、标准驱动测试数据包 tool 和 evidence package 导出 tool。
...
解压后包含 `.dataspec/DATABASE_RULES.md`、`.dataspec/field-catalog.json`、`.dataspec/field-catalog.schema.json`、`.dataspec/table-standards.json`、`.dataspec/field-knowledge-cards.json`、`.dataspec/field-semantics.json`、`.dataspec/metrics.json`、`.dataspec/schema-registry.json`、`.dataspec/capabilities.json`、`.dataspec/usage-examples.json`、`.dataspec/rules.yaml`、`.dataspec/prompts.md`、`.dataspec/workflows.md`、`.dataspec/examples/good.sql`、`.dataspec/examples/bad.sql` 和 `AGENTS.md.fragment`。可将这些文件复制到业务项目，让 Codex/Cursor/Claude Code 等 agent 在建表或评审 SQL 前先读取能力清单、字段标准、表结构标准、字段语义证据、使用示例/反例、契约和规则。`.dataspec/capabilities.json` 会包含 `standard-evidence`，离线 agent 可据此发现 `GET /api/standard-evidence`；该文件只描述入口，不包含字段证据结果、raw SQL、AI payload、候选 raw evidence、raw source metadata 或凭据。
...
按需导出支持 `scope=all|field|domain|tag|table|changed|business-object|table-template`，可叠加 `query`、`status` 和 `limit`。裁剪后的 `field-catalog.json` 会输出 `contextScope` 摘要和字段级 `matchReasons`，说明命中条件、字段总数、命中数量、返回数量和缺失或截断提示；`.dataspec/table-standards.json` 会在 `business-object` 或 `table-template` scope 下只保留匹配对象、关联模板、关系 edge 和结构标准摘要；`.dataspec/README.md` 会标明当前包是完整包还是按需包。前端“AI Context”页面也提供同样的范围、关键词、状态和上限筛选，并可选择“当前标准”或历史快照，预览与下载共用同一组条件。

导出前可先做 AI Context 预算计划：`POST /api/ai-context/budget/plan` 和 CLI `context-budget plan` 会基于 `projectId`、`tokenBudget`、可选 `scope/query/status/limit`、任务类型或目标表/文件提示，返回 `selectedArtifacts`、`droppedArtifacts`、`estimation.selectedEstimatedTokens`、`estimation.totalEstimatedTokens`、`qualityRisk`、`fallbackSteps`、`recommendedExportParams` 和 `recommendedNextActions`。预算计划是只读 preflight：不生成 zip、不写 `.dataspec/context/` 缓存、不修改项目状态、不调用外部 LLM 或 tokenizer，也不会返回 raw token、password、Authorization、完整 JDBC URL、DSN、连接串或业务数据行。CLI `context-quality check` 可本地读取已导出的 `.dataspec/context/`、AI Context zip 或预算 plan JSON，输出 `contextQualityScore`、`qualityLevel`、缺失关键资源、截断资源、分类覆盖、任务适配提示和下一步动作；第一版不新增后端 API、不调用外部 LLM、不写缓存。前端“AI Context”页面可查看预算预览，并且只有点击“一键填充”时才会应用推荐导出参数。
...
字段语义主题包会额外导出字段命名翻译、派生/单位/时间粒度/聚合口径、枚举值 lifecycle、字段知识卡和指标定义映射。`.dataspec/field-knowledge-cards.json` 聚合字段身份、格式、usage contract、语义规则、枚举提示、命名证据、指标引用和风险说明；`.dataspec/field-semantics.json` 记录 source-of-truth、禁用翻译、推荐使用场景和反例；`.dataspec/metrics.json` 记录 metricKey、定义、measure/dimension 字段、过滤、聚合、时间粒度和示例 SQL 摘要。示例 SQL 永远是说明性文本，不连接业务数据库、不验证计算结果、不自动改生产 SQL。

导出包还会包含 `.dataspec/usage-examples.json`，按优先级裁剪项目中启用的标准使用示例与反例。完整包会导出少量高价值 `FIELD/RULE/TEMPLATE/GENERAL` 示例；按需包会优先保留匹配字段的 `FIELD` 示例，并只带入命中 query 的通用、规则或模板示例。AI 应优先模仿 `GOOD` 示例，避开同 scope 的 `BAD` 反模式；历史快照导出仍使用当前项目的示例库元数据，并在文件中标记 `snapshotBound=false`。
...
标准驱动测试数据包用于从字段标准、枚举、格式约束、敏感标记和轻量对象提示生成 safe-to-share 的测试样例，方便 AI 生成单测、mock、seed 草稿或数据质量反例。消费端兼容检查用于在契约变更后本地确认 DataSpec 自有 API、CLI、MCP、AI Context 和 Schema Registry golden payload 仍可被消费端解析。

```bash
curl -X POST "http://localhost:8090/api/test-data/package/generate" \
  -H "Content-Type: application/json" \
  -d '{"projectId":1,"fieldNames":["order_id"],"maxCasesPerField":3}'

node tools/dataspec-cli.mjs test-data generate --project 1 --field order_id --format json
node tools/dataspec-cli.mjs test-data generate --project 1 --format text

node tools/dataspec-cli.mjs consumer-compat check --format json
node tools/dataspec-consumer-compat-check.mjs --format json
```

测试数据包返回 `kind/schemaVersion/projectId/specHash/generationParams/sourceSummary/testDataCases/seedProfiles/mockPayloads/coverageReport/diagnostics/safety/nextActions`。`testDataCases[]` 覆盖 valid、invalid 和 boundary case；`seedProfiles[]` 只输出 SQL/CSV 草稿，并明确 `executable=false` 或 `requiresReview=true`；`safety` 会声明 `readOnly=true`、`writesProject=false`、`writesBusinessRepo=false`、`containsRealBusinessRows=false`、`externalNetworkUsed=false`、`externalLlmUsed=false`。生成器不写 DataSpec 状态、不写业务仓库、不连接源数据库、不采样真实业务数据行。

消费端兼容检查返回 `kind/schemaVersion/suiteVersion/minimumSupportedVersion/status/summary/goldenPayloads/breakingRules/adapterResults/diagnostics/nextActions`。它只读取本地 fixture、descriptor 和 schema 文档，不要求 DataSpec 服务启动，不连接数据库，不调用外部网络或 LLM；发现 stable field、descriptor、output shape、安全 metadata 或示例脱敏漂移时会报告 `BREAKING` 并指出 adapter、contract path、rule 和 migration hint。
...
启动包是 AI agent 进入业务仓库后的第一跳，用来一次性回答“当前项目是谁、服务在哪、token 是否存在、标准版本是什么、能调用哪些能力、下一步该跑什么命令”。它只读，不会执行 lint、导出 Context、连接数据库、生成 DDL 或写入标准。

```bash
curl "http://localhost:8090/api/bootstrap/session?projectId=1&server=http://localhost:8090"

node tools/dataspec-cli.mjs bootstrap --project 1 --format json
node tools/dataspec-cli.mjs bootstrap --project 1 --format text
```
...
AI 任务卡用于把一次 AI/DataSpec 工作记录成可恢复的本地 JSON 或 Markdown，不替代 workflow recipe，也不会自动执行任何步骤。

```bash
node tools/dataspec-cli.mjs task-card create --workflow create-table --goal "创建订单表" --project 1 --input businessDescription=订单表 --format json --output .dataspec/task-card.json
node tools/dataspec-cli.mjs task-card show --file .dataspec/task-card.json --format markdown
node tools/dataspec-cli.mjs task-card update --file .dataspec/task-card.json --step precheck-1 --status DONE --artifact dataspec-ai-context.zip
```

任务卡字段包含 `goal`、`inputs`、`currentStep`、`steps`、`allowedActions`、`artifacts`、`resumeCommand`、`validationCommands`、`stopConditions`、`risks` 和 `nextActions`。MCP 可调用 `create_task_card` 与 `render_task_card` 获取相同协议；前端提供 `taskCardDisplay` 展示/Markdown 工具。任务卡会脱敏 token、Authorization、password 和完整 JDBC URL，但仍建议只写入任务需要的非敏感输入。
...
版本兼容握手是 AI/CLI/MCP 的只读 preflight 入口，用来判断本地客户端是否能安全消费当前服务端能力。后端提供 `GET /api/capabilities/version?client=cli&clientVersion=0.1.0`，返回 `serverVersion`、`apiSchemaHash`、`minCliVersion`、`supportedCapabilities`、`deprecatedFields`、`compatibility`、`upgradeHints` 和 `generatedAt`；接口不读取项目业务数据、不连接数据库、不写入 DataSpec 状态，也不返回 token、password、Authorization、JDBC URL 或 DSN。

```bash
curl "http://localhost:8090/api/capabilities/version?client=cli&clientVersion=0.1.0"

node tools/dataspec-cli.mjs compat check --format json
node tools/dataspec-cli.mjs doctor --format json
```

CLI `compat check` 在兼容时返回 `0`，服务端报告 `compatibility.compatible=false` 时返回 `1`，服务不可达或响应错误时返回 `2` 并输出脱敏 JSON 诊断。`doctor --format json` 会包含 `compatibility` 检查项，但兼容 endpoint 不可用时仍保留既有 server、auth、project、defaultPaths、OpenAPI 和 context-cache 检查。MCP 可读取全局 resource `dataspec://version-compatibility`，返回同一份 JSON text 和 `structuredContent`，建议 AI 在版本敏感 workflow 前先读取。
...
AI task run 用于记录 AI 面向任务的运行状态和恢复建议。第一版不引入外部队列、后台 worker 或自动重放，只持久化项目级诊断信息：`taskType`、`status`、`sourceType/sourceId`、`inputHash`、`idempotencyKey`、`stepStatus`、`retryable`、`failedStep`、`resumeCommand`、`nextAction`、`partialArtifacts`、`expiresAt` 和脱敏 metadata。

后端 API：

```bash
curl "http://localhost:8090/api/ai-task-runs?projectId=1&current=1&size=10"
curl "http://localhost:8090/api/ai-task-runs/recent-failures?projectId=1&limit=5"
curl "http://localhost:8090/api/ai-task-runs/91?projectId=1"
```

CLI/MCP：

```bash
node tools/dataspec-cli.mjs task list --project 1 --format json
node tools/dataspec-cli.mjs task failures --project 1 --format json
node tools/dataspec-cli.mjs task show 91 --project 1 --format json
```

AI 批量 SQL lint 已首批接入 task run。交付包会带 `taskRun` 摘要；部分失败或失败时，前端“AI 批量任务”页会显示最近可恢复任务、失败步骤、retryable 状态、partial artifacts 和可复制恢复命令。metadata 与 artifact 摘要写入前会脱敏，不保存 token、password、Authorization、完整 JDBC URL 或业务数据行。

## AI 执行证据包

AI 执行证据包用于把一次 SQL 修复、AI job、AI 批量任务、AI task run 或覆盖率分析整理成可复制、可下载、可继续交给下游 AI 的只读交付物。JSON 包稳定包含 `kind`、`schemaVersion`、`packageId`、`projectId`、`generatedAt`、`source`、`standardSnapshot`、`inputsSummary`、`outputsSummary`、`validationSummary`、`artifacts`、`nextActions`、`suggestedCommands` 和 `diagnostics`；zip 包固定包含 `evidence.json`、`summary.md` 和 `README.md`。
...
curl -X POST "http://localhost:8090/api/evidence-packages" \
  -H "Content-Type: application/json" \
  -d '{"projectId":1,"sourceType":"AI_TASK_RUN","sourceId":91}'
```
...
curl -X POST "http://localhost:8090/api/reverse-import/database/browser" \
  -H "Content-Type: application/json" \
  -d '{"projectId":1,"databaseType":"postgresql","host":"localhost","port":5432,"databaseName":"demo","schemaName":"public","username":"readonly","password":"<password>","tableNames":["user_order"]}'
...
curl -X POST "http://localhost:8090/api/project-backups/restore/apply" \
  -H "Content-Type: application/json" \
  -H "Idempotency-Key: restore-20260705-001" \
  -d '{"overwrite":false,"dryRunToken":"<preview.data.dryRunToken>","backupPackage":{"schemaVersion":1,"assets":{},"packageHash":"..."}}'
...
个人工作台提供项目级摘要：任务入口、最近任务、标准字段数、代码集数、规则数、禁用词数、SQL 检查数、字段命中率、最近检查、问题趋势和最近活动。任务入口覆盖数据库反向导入、SQL 校验、覆盖率报告、字段维护、DDL 生成、AI Context 导出和 API Token 管理；最近任务只保存在当前浏览器 localStorage，不写入后端。顶部命令面板可从任意页面搜索常用入口，并基于当前项目恢复最近 SQL 检查、反向导入批次和 AI job。
...
# 输出 IDE Problem Matcher 可解析的 file:line:column 行格式
node tools/dataspec-cli.mjs lint-changed --format text

# 显式安装 DataSpec 管理的 pre-commit hook；可选生成 VS Code task/problem matcher
node tools/dataspec-cli.mjs install-hook --with-vscode --format json

# 只读扫描业务仓库字段引用，输出文件位置、置信度和重命名风险；默认使用 defaultPaths，也可显式传 --path
node tools/dataspec-cli.mjs index-refs --field phone --alias mobile_phone --path sql --path models --format json

# 执行项目级标准质量门禁；PASS 退出码 0，FAIL 退出码 1，参数/服务错误退出码 2
node tools/dataspec-cli.mjs quality-gate check --project 1 --format json
...
# 导出前预估预算、裁剪取舍和质量风险；只读，不写 zip/cache
node tools/dataspec-cli.mjs context-budget plan --project 1 --token-budget 8000 --scope field --query "用户手机号" --format json

# 本地评估已导出 AI Context 或预算 plan 是否足够当前 AI 任务；只读，不调用后端
node tools/dataspec-cli.mjs context-quality check --context-dir .dataspec/context --format json
...
# 读取字段语义知识卡、语义规则和指标口径；只读，不执行 SQL 或真实计算
node tools/dataspec-cli.mjs field-knowledge list --project 1 --query "订单金额" --format json
node tools/dataspec-cli.mjs field-knowledge show --project 1 --field-id 10 --format json
node tools/dataspec-cli.mjs field-semantics list --project 1 --field-id 10 --format json
node tools/dataspec-cli.mjs metric-definitions list --project 1 --field-id 10 --format json
...
# 生成标准驱动测试数据包；只读，不采样真实业务数据行
node tools/dataspec-cli.mjs test-data generate --project 1 --field order_id --format json
node tools/dataspec-cli.mjs test-data generate --project 1 --format text

# 本地检查 DataSpec 自有消费端契约兼容性；不要求后端服务启动
node tools/dataspec-cli.mjs consumer-compat check --format json
...
node tools/dataspec-cli.mjs evidence export --project 1 --source-type AI_TASK_RUN --source-id 91 --format json

# 查询 AI task run 状态和最近失败恢复命令
node tools/dataspec-cli.mjs task list --project 1 --format json
node tools/dataspec-cli.mjs task failures --project 1 --limit 5 --format json
node tools/dataspec-cli.mjs task show 91 --project 1 --format json
...
# 检查本地 CLI/MCP 与服务端版本兼容性；不兼容返回 1，服务不可达返回 2
node tools/dataspec-cli.mjs compat check --format json
...
node tools/dataspec-cli.mjs workflow show standard-evidence-review --format json
...
# 调试 SQL 规则匹配，不保存检查记录
node tools/dataspec-cli.mjs lint-debug examples/bad-example.sql --project 1 --format json
...
- resources：`version-compatibility`、`capability-catalog`、`session-bootstrap`、`session-state`、`field-catalog`、`table-standards`、`field-knowledge-cards`、`field-semantics`、`metric-definitions`、`database-rules`、`rules-yaml`、`workflow-recipes`、`agent-guidance-pack`、`ai-task-profiles`、`schema-registry`、`ai-task-runs`，URI 形如 `dataspec://project/1/session-bootstrap`；也支持只读全局版本兼容握手 `dataspec://version-compatibility` 和全局能力清单 `dataspec://capability-catalog`。
- resource templates：`resources/templates/list` 返回 `dataspec://project/{projectId}/...` 模板，覆盖 session bootstrap、session state、capability catalog、schema registry、field catalog、workflow recipes、AI task profiles 和 agent guidance pack，便于未固定项目的 MCP client 先发现可读资源。
...
- tools：`get_session_bootstrap`、`get_session_state`、`create_task_card`、`render_task_card`、`lint_sql`、`get_field_catalog`、`get_table_standards`、`get_field_knowledge_cards`、`get_field_semantics`、`get_metric_definitions`、`search_field_catalog`、`suggest_fields`、`search_fields`、`generate_table_ddl`、`get_ai_task_run`、`export_evidence_package`；`tools/list` 会在每个工具描述中暴露本地 `safety` metadata，MCP client 可先判断只读、写项目、dry-run、幂等 key 和敏感输入边界；`get_session_state` 只读聚合本地配置、context cache 元数据、当前 profile、最近 task run 入口和安全默认值，projectless 时返回 `BLOCKED` 并提示提供 `projectId` 或更新 `.dataspec/config.json`；`get_session_bootstrap` 返回与 API/CLI 一致的启动包，未配置默认项目时也可先返回 `SELECT_PROJECT` nextAction；`create_task_card` 和 `render_task_card` 只生成或渲染本地任务卡，不执行 workflow；`lint_sql`、`get_field_catalog`、`get_table_standards`、`get_field_knowledge_cards`、`get_field_semantics`、`get_metric_definitions` 和 `search_field_catalog` 可接收 `profileId/taskType` hint，显式工具参数仍优先于默认 profile；`get_field_catalog` 可传 `scope/query/status/limit`，`get_table_standards` 可按业务对象或表模板读取只读表结构标准，字段语义工具只读返回知识卡、语义规则和指标口径 metadata guidance，不执行 SQL 或真实计算，`search_field_catalog` 默认按当前关键词读取较小字段目录，`search_fields` 调用 `/api/fields/search` 并返回字段、分数、命中原因和下一步建议；`generate_table_ddl` 会保留后端返回的 `structureSummary`；`get_ai_task_run` 按项目读取任务详情和恢复命令；`lint_sql` 返回结构化 lint 结果，SQL 存在 ERROR 时仍视为工具调用成功；`export_evidence_package` 返回 `structuredContent` 与可解析 JSON text，用于交付前导出只读证据包。
...
第一版覆盖 token、项目权限、projectId、资源不存在、SQL 输入、数据库连接、参数校验、写入安全参数和内部错误等高频场景。缺少强制幂等 key 的写入会返回 `IDEMPOTENCY_KEY_REQUIRED`，并在 `error` / `DataSpecError` / `error.data.dataspecError` 中保留 `missing`、`operation`、`safety` 和 `nextActions`。CLI 在保留 `错误: ...` 人类可读行的同时，会额外输出 `DataSpecError: {...}`；MCP 会把同类诊断放到 JSON-RPC `error.data.dataspecError`。该对象用于诊断和恢复建议，不暴露内部异常堆栈，也不改变原有 HTTP 状态语义。
...
# 检查 README/TODO/OpenSpec、AI workflow recipe 文档清单和链接是否漂移
node tools/dataspec-status-check.mjs --format json

# 开工前检查 active OpenSpec change 是否缺验收、边界、影响规格或验证计划
node tools/dataspec-openspec-readiness.mjs --change add-your-change --format json

# 检查 CLI/MCP 契约 fixture、MCP descriptors 和安全 metadata 是否对齐
node tools/dataspec-cli-mcp-contract-check.mjs --format json

# 检查 DataSpec 自有消费端 golden payload 与 breaking rules 是否兼容
node tools/dataspec-consumer-compat-check.mjs --format json
...
# 可选真实数据库集成测试；需要本机 Docker 可用，会启动 PostgreSQL/MySQL Testcontainers
mvn test -Pdb-integration

# 前端源码级 smoke、类型检查与生产构建；不启动真实浏览器
cd ../dataspec-web
pnpm test
pnpm build

# 可选浏览器级 E2E 验收；失败时保留截图、trace、video 和当前 URL
pnpm test:e2e

# 查看失败 trace，trace.zip 位于 test-results/e2e 下对应失败用例目录
...
# CLI/MCP 单元测试
cd ..
node --test tools/dataspec-config.test.mjs tools/dataspec-cli.test.mjs tools/dataspec-mcp.test.mjs tools/dataspec-cli-mcp-contract-check.test.mjs tools/dataspec-consumer-compat-check.test.mjs tools/dataspec-verify-advisor.test.mjs tools/dataspec-status-check.test.mjs tools/dataspec-todo-openspec-handoff.test.mjs tools/dataspec-openspec-readiness.test.mjs
...
`node tools/dataspec-verify-advisor.mjs --changed --format json` 会按变更路径推荐最小验证集，输出命令、原因、工作目录、预计耗时和下一步动作；它只给建议，不自动执行命令。`node tools/dataspec-status-check.mjs --format json` 会检查 README/TODO/OpenSpec 状态漂移、完成项残留缺口、active/archive/main spec 同步、AI workflow recipe 文档清单和 Markdown 相对链接；它只做确定性本地检查，不联网、不读取业务数据。`node tools/dataspec-openspec-readiness.mjs --change <change-id> --format json` 会检查 active OpenSpec change 是否缺验收标准、边界、有效 spec delta、Impact 内容、验证命令、占位符或人工确认问题，输出 `readinessScore`、`missingFacts`、`validationPlan` 和 `humanQuestions`，并在验证命令里脱敏 token、password、JDBC URL、DSN 和 URL userinfo；它只读扫描 artifacts，不执行实现、测试、提交或归档。`node tools/dataspec-cli-mcp-contract-check.mjs --format json` 会检查 CLI/MCP contract fixture、MCP descriptors 和安全 metadata 是否对齐，不访问真实服务。`node tools/dataspec-consumer-compat-check.mjs --format json` 会本地检查消费端兼容 suite、golden payload、adapter results、breaking rules 和 fixture secret safety，不要求服务启动。`node tools/dataspec-todo-openspec-handoff.mjs --item P6-48 --dry-run --format json` 会从 TODO 条目生成 OpenSpec 草稿计划，确认后去掉 `--dry-run` 写入 `openspec/changes/<change-id>/`；它只生成 proposal/design/spec/tasks 草稿，不实现代码、不提交、不归档。后端 `mvn test` 不运行 `*IT.java` Testcontainers 矩阵、不要求 Docker，已包含核心 fixture/golden 回归测试、Prompt 模板 registry/eval、AI contract fixtures、AI evidence package、AI 可读错误诊断、幂等写保护、标准变更 What-if 预览、标准测试数据包和合成性能基线，覆盖 PostgreSQL/MySQL SQL 样例、fixedSql golden 输出、Prompt golden 输出、反向导入 metadata 预览摘要、Schema Registry、AI Context、lint/fixedSql、字段推荐、字段检索、DDL 预览、执行证据包稳定字段与脱敏、写入重复 key/任务锁冲突/AI job 去重、字段/规则变更预览风险与回退提示，以及千级字段库下的字段分组、字段推荐、AI Context 字段目录和反向导入 compare。`mvn test -Pdb-integration` 是可选真实数据库集成测试入口，会通过 Testcontainers 启动 PostgreSQL/MySQL，加载 schema-only fixture，验证表列表、metadata dump/browser、compare、coverage 和只读连接诊断；本机未安装或未启动 Docker 时该命令会在容器启动阶段失败，应先修复 Docker 环境再用于真实方言验收。前端 `pnpm test` 已包含关键流程源码级冒烟门禁，覆盖路由导航、项目选择、统一请求状态、SQL 校验 fixedSql/记录/evidence 入口、数据库反向导入、字段库检索命中原因、标准变更预览确认、标准候选、筛选与批量维护、DDL 生成、AI Context、覆盖率报告 evidence 入口、AI 回放、AI 反馈、AI 批量任务 evidence 入口、项目备份恢复和新增 API schema wiring 的核心页面/API 耦合，以及关键按钮、空状态和失败重试入口；它不需要浏览器、后端服务或截图依赖。前端 `pnpm test:e2e` 是可选浏览器级验收入口，会启动 Vite dev server 并通过 Playwright 覆盖创建/选择项目、SQL 校验 fixedSql、检查记录详情、数据库直连预览、字段库筛选和 AI Context 预览；失败 artifacts 写入 `dataspec-web/test-results/e2e` 和 `dataspec-web/playwright-report`。Windows 环境默认优先使用本机 Microsoft Edge，其他平台默认使用 Playwright Chromium；如需显式改用 bundled browser，先运行 `pnpm exec playwright install chromium`，再设置 `DATASPEC_E2E_BROWSER_CHANNEL=bundled` 后执行 `pnpm test:e2e`。`node --test` 覆盖 CLI/MCP JSON 契约、CLI/MCP contract fixture、consumer compatibility suite、Prompt fixture 脚本、本地 smoke 脚本、验证建议工具、状态一致性检查、TODO 到 OpenSpec 交接助手和 OpenSpec readiness 检查，包括 contract list/show/check、consumer-compat check、test-data generate、evidence export、schema-registry resource、workflow recipes、resource/tool `structuredContent`、字段标准检索、可解析文本内容、Prompt golden marker、Idempotency-Key 透传、API 失败时的 `DataSpecError` / `error.data.dataspecError` 诊断透传、变更路径到验证命令的推荐规则、README/TODO/OpenSpec 状态漂移检测、TODO 条目到 OpenSpec 草稿的字段保留和覆盖保护、change 准备度评分与缺口诊断，以及本地启动包的参数解析、输出结构、敏感信息脱敏和 compose/Vite 代理契约。`npx openspec validate --all` 用于校验当前 `openspec/specs/` 主规格和仍处于 active 状态的 change；已完成 change 应归档到 `openspec/changes/archive/`，主规格作为后续开发的权威入口。
...
| aitaskrun | /api/ai-task-runs | AI 任务状态、失败诊断和恢复命令 |
...
| tablemodel | /api/business-objects | 业务对象、表结构标准和只读关系摘要 |
...
- [x] 全局命令面板和最近操作续跑，支持顶部入口、`Ctrl/⌘+K`、项目建议、页面搜索和最近 SQL/反向导入/AI 作业恢复
...
- [x] 标准质量门禁，支持项目级阈值配置、评估 `PASS/FAIL/DISABLED`、前端状态展示和 CLI `quality-gate check`
...
- [x] 字段语义、枚举生命周期、字段知识卡、指标口径和命名翻译辅助第一版，支持后端维护 API、前端维护/只读入口、AI Context 三个语义 artifact、Schema Registry、CLI/MCP 只读查询和 secret-safe 边界
- [x] 业务对象与表结构标准第一版，支持表模板关联、主键/唯一键/索引/外键/策略摘要、AI Context table standards 和只读关系摘要
- [x] DDL 生成 API/CLI/MCP 和前端预览下载，包含结构标准 `structureSummary` 摘要
...
- [x] DataSpec CLI：`bootstrap`、`task-card create/show/update`、`doctor`、`compat check`、`consumer-compat check`、`profile list/show`、`workflow list/show`、`task list/failures/show`、`quality-gate check`、`contract list/show/check`、`evidence export`、`lint`、`lint-files`、`changed`、`lint-changed`、`install-hook`、`index-refs`、`code-patch plan`、`review-pr`、`export-context`、`table-standards list/show`、`suggest-field`、`search-fields`、`generate-ddl`、`synthetic-examples generate`、`test-data generate`、`contract-import preview`，支持 `.dataspec/config.json` 默认项目配置、AI profile 默认值、业务仓库 git 变更感知、本地 pre-commit/VS Code SQL 检查模板、业务代码字段引用索引、字段变更 Patch Plan、版本兼容诊断、标准质量门禁检查、AI task card 本地交接、AI batch delivery package 文件输出、AI task run 恢复诊断、按需/历史快照/表结构 Context 导出、合成标准样例包、标准测试数据包、消费端兼容检查、契约候选导入预览、AI evidence package 导出和 PR diff inline/fallback SQL Review
- [x] DataSpec MCP Server：resources、`version-compatibility`、`session-bootstrap`、`session-state`、`consumer-compatibility-suite`、`table-standards`、`ai-task-profiles`、`ai-task-runs`、`workflow-recipes`、`schema-registry`、prompts、`get_session_bootstrap`、`get_session_state`、`create_task_card`、`render_task_card`、`lint_sql`、`get_field_catalog`、`get_table_standards`、`search_field_catalog`、`search_fields`、`suggest_fields`、`generate_table_ddl`、`generate_test_data_package`、`check_consumer_compatibility`、`get_ai_task_run`、`export_evidence_package`，支持 `.dataspec/config.json` 默认项目配置、只读会话状态记忆、task card 本地交接、表结构标准读取、消费端兼容检查和 profile hint

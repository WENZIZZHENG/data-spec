# fixed-sql-policy Specification

## Purpose
Define request-level controls and machine-readable explanations for deterministic `fixedSql` generation, so AI agents and frontends can preview, filter, or disable automatic SQL fixes without changing project rule configuration or writing back to business repositories.
## Requirements

...
test('keeps global project selector backed by the project store', () => {
  ...
  assertContains(app, [
    ...
    'projectStore.currentProjectId ?? 0',
...
test('keeps database reverse import and comparison flow wired', () => {
  ...
  assertContains(api, [
    ...
    "request.post<unknown, DatabaseSchemaChangePlan>('/reverse-import/database/schema-plan'",
  ...
  assertContains(schema, [
    ...
    '/** @description 当前源库 schema-only metadata hash',
    'currentSchemaHash?: string;',
    '/** @description 合并后的 dry-run SQL 草案',
    'migrationSql?: string;',
    '/** @description 全局阻塞原因',
    'blockedReasons?: string[];',

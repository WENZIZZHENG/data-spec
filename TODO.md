...
- `P6-120` 推荐质量与 AI 场景回归基线已完成；当前高优先级转为字段库性能和统一评审证据。
- 2026-07-14 完成 `P6-120` 后，当前剩余 7 个实施主题：5 个进入近期队列，2 个等待业务触发；`P6-191` 继续承接第三轮功能评审建议。
- 近期优先顺序为 `P6-86`、`P6-191`、`P6-137`、`P6-111`、`P6-85`，串行约 10.5-17 个工作日。
...
- 已完成 P5/P6 详情见 [docs/archive/todo-completed-p5-p6.md](docs/archive/todo-completed-p5-p6.md)，当前归档 141 个待办编号。
...
1. `P6-86`：字段库服务端分页、搜索防抖和大数据量性能闭环，约 2-3 个工作日。
2. `P6-191`：统一 Finding/Evidence 与 AI/PR 评审闭环，约 3-5 个工作日。
3. `P6-137`：`.dataspec/config.json` Schema 与编辑器提示，约 0.5-1 个工作日。
4. `P6-111`：先接一个可追溯候选来源，优先承接未知词和缩写歧义，约 2-3 个工作日。
5. `P6-85`：演示项目 dry-run 清理与完整重建，约 3-5 个工作日。

开工前读取候选主题的近期范围和边界，再按任务风险决定快速、常规或 OpenSpec 流程。暂缓主题只有触发条件成立时才进入队列。

## 待办入口

- [P6 精简候选池](docs/todo-p6-candidates.md)：7 个仍值得实施的主题、第一版边界和启动条件。
- [P6 候选价值评审](docs/todo-p6-candidate-review.md)：记录 60 项候选的保留、合并、完成覆盖和删除依据。
- [P6 剩余时间评估](docs/todo-p6-remaining-estimates.md)：按 7 个实施主题估算开发时间。
...
`P6-86`、`P6-191`、`P6-137`、`P6-111`、`P6-85`。
